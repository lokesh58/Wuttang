...
void initHashTable(Hash_Table &hashTable, const int MB) {
  ...
  if (hashTable.pTable == nullptr) {
    ...
    // std::cout << "HashTable init complete with " << hashTable.numEntries << "
    // entries\n";
  }
}
...
void clearHashTable(Hash_Table &hashTable) {
  ...
  for (hashEntry = hashTable.pTable;
       hashEntry < hashTable.pTable + hashTable.numEntries; ++hashEntry) {
...
bool probeHashEntry(Position &pos, int &move, int &score, int alpha, int beta,
                    int depth) {
  ...
  if (pos.hashTable.pTable[index].posKey == pos.posKey) {
    ...
    if (pos.hashTable.pTable[index].depth >= depth) {
      ...
      assert(pos.hashTable.pTable[index].depth >= 0 &&
...
void storeHashEntry(Position &pos, int move, int score, const int flags,
                    const int depth) {
  int index = pos.posKey % pos.hashTable.numEntries;
