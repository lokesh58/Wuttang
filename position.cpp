...
void Position::parseFEN(const std::string fenStr) {
  ...
  while (rank >= rank_1) {
    ...
    while (cnt > 0) {
      if (piece != EMPTY) pieces[fr2sq(file, rank)] = piece;
      ++file;
      --cnt;
    }
    ++fen;
...
void Position::display() const {
  ...
  std::cout << "Side : " << sideChar[side] << '\n'
            << "EnPassSq : " << enPass_sq << '\n'
            << "CastleRights : " << (castleRights & WKCA ? 'K' : '-')
            << (castleRights & WQCA ? 'Q' : '-')
            << (castleRights & BKCA ? 'k' : '-')
            << (castleRights & BQCA ? 'q' : '-') << '\n'
...
bool Position::checkBoard() const {
  ...
  assert(enPass_sq == NO_SQ || (rankSq(enPass_sq) == rank_6 && side == white) ||
         (rankSq(enPass_sq) == rank_3 && side == black));
...
bool Position::isSqAttacked(const int sq, const int side) const {
  ...
  // pawns
  if (side == white) {
    if (pieces[sq - 11] == wP || pieces[sq - 9] == wP) return true;
  } else {
    if (pieces[sq + 11] == bP || pieces[sq + 9] == bP) return true;
  }
